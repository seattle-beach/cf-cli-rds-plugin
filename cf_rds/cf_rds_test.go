		...
		Describe("Run", func() {
			Context("register", func() {
				...
				BeforeEach(func() {
					...
					args = []string{"rds-register", "name", "--uri", "postgres://user:pwd@example.com:5432/database"}
				})
				...
				Context("error cases", func() {
					It("returns an error if there are not enough arguments", func() {
						args = []string{"rds-register", "name"}
						p.Run(conn, args)

						Expect(ui.Err).To(MatchError(ContainSubstring("cf rds-register SERVICE_NAME --uri URI")))
					})

					It("returns an error if the --uri option flag is not provided", func() {
						args = []string{"rds-register", "name", "--foo", "postgres://foo"}
						p.Run(conn, args)
					...
					Commands: []plugin.Command{
						{
							...
							UsageDetails: plugin.Usage{
								Usage: "cf rds-register SERVICE_NAME --uri URI",
							},
						},
						{
							Name:     "rds-create",
							...
							UsageDetails: plugin.Usage{
								Usage: "cf rds-create SERVICE_NAME",
							},
						},
						{
							Name:     "rds-refresh",
							...
							UsageDetails: plugin.Usage{
								Usage: "cf rds-refresh SERVICE_NAME",
							},
						},
					},
				}))
